...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, RouterLinkActive, IfLoggedDirective],
  template: `
    <div class="navbar bg-base-100 sticky top-0 z-10">
      ...
      <div class="flex gap-2">
        <ng-container *appIfLogged>
          <a routerLink="/cms">cms</a>
          <div>{{ displayName() }}</div>
          <button (click)="logout()">Quit</button>
        </ng-container>
        <a routerLink="/login" *appIfLogged="true">login</a>
      </div>
      <div class="flex-none">
        <div class="dropdown dropdown-end">
          <div tabindex="0" role="button" class="btn btn-ghost btn-circle">
            <div class="indicator">
              ...
              <span class="badge badge-sm indicator-item">{{totalCartItems()}} </span>
            </div>
          </div>
